...
#'
#' @param dist A ser passado para função erros. O comportamento do erro do modelo. O nome (string) de qualqer função geradora de do pacote {stats}, ou "arma" para modelar o erro como um arma (o erro desse modelo é uma normal padrão).
...
#' @param H0 Uma lista com a hipótese nula sobre os parâmetros intercepto, ar e ma passados em model. Formato: list(intercepto = ...., ar=c(....), ma=c(....))
...
test_raiz_unit <- function(Y, dist, args, H0, S, a = 0.05, trend = FALSE, sazon = FALSE, cicle = FALSE) {
  # Temos tendencia etc no modelo?:
  cond.t <- !identical(trend, FALSE)
  cond.s <- !identical(sazon, FALSE)
  cond.c <- !identical(cicle, FALSE)
  cond <- H0$intercepto != 0

  k <- as.numeric(H0$intercepto != 0) + length(H0$ar) + length(H0$ma) # numero de coefficientes
  order <- c(length(H0$ar), 0, length(H0$ma)) # ordem do ARMA

  if (cond) {
    coefs <- c(H0$ar, H0$ma, H0$intercepto)
  } # coeficientes da H0 com intercepto
  else {
    coefs <- c(H0$ar, H0$ma)
  } # sem

  # Monte carlo, definir distribuicao empírica da estatistica t para cada parâmetro
  tstat <- numeric()
  for (i in 1:S) {
    y <- simular_serie(dist, args, H0)
    mod <- try(stats::arima(y, order, include.mean = cond))
  ...
  tstat <- stats::na.omit(apply(tstat, 2, as.numeric))


  # Ver o t-valor da estimação coma serie original, controlando por tendencia e etc
  if (cond.t) {
    f <- tendencia(y, trend)
  } else {
    f <- 0
  ...
  mod <- stats::arima(y - f - s - c, order, include.mean = cond)
  ...
  # Ver qual e o menor nível de significancia que o parametro k aceita
  passou <- numeric()
  for (i in 1:k) {
    passou[i] <- names(which(T[i] < stats::quantile(tstat[, i], seq(0, 1, 0.005)))[1])
  }

  # Graficos da distribuicao da estatistica t
  val.crit <- numeric()
  for (i in 1:k) {
    val.crit <- cbind(val.crit, stats::quantile(tstat[, i], 0.05))
  } # expandir pra teste uni e bi caudais
  ...
  return(list(
    #Graphs" = graphs,
    "Val.crít" = apply(tstat, 2, stats::quantile, probs = a),
    "T.stat" = T.stat
  ))
}
