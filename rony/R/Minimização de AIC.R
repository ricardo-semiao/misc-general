...
min.AIC = function(Y, pmax=6, qmax=6, dmax=2){
  AIC = list()
  for(d in 0:dmax){
  ...
  for(d in 0:dmax){
    for(p in 0:pmax){
      for(q in 0:qmax){
        AIC[[d+1]][p+1,q+1] = try(arima(Y, c(p,d,q))$aic, silent=TRUE)}}
  ...
  if(dmax>0){i=0
    while(i < dmax){
      if(min(AIC[[i+1]]) < min(AIC[[i+2]])) {d=i} else {d=i+1}
  i=i+1}}
...
#min.AIC(Y, pmax=3, qmax=3, dmax=1)
